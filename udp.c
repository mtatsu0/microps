...
udp_close(int id)
{
    ...
    pcb = udp_pcb_get(id);
    if (!pcb) {
        errorf("designated id is not used.");
        mutex_unlock(&mutex);
        return -1;
    }
    udp_pcb_release(pcb);
...
udp_bind(int id, struct ip_endpoint *local)
{
    ...
    if (exist) {
        errorf("designated endpoint is already used.");
        mutex_unlock(&mutex);
        return -1;
    }
    pcb->local = *local;
...
udp_sendto(int id, uint8_t *data, size_t len, struct ip_endpoint *foreign)
{
    ...
    if (!pcb->local.port) {
        ...
        if (!pcb->local.port) {
            debugf("failed to dyanmic assign local port, addr=%s", ip_addr_ntop(local.addr, addr, sizeof(addr)));
            mutex_unlock(&mutex);
            return -1;
        }
    }
