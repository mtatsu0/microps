...
icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    // Exercise 10-1
    if (len < sizeof(*hdr)) {
        errorf("too short icmp");
        return;
    }

    hdr = (struct icmp_hdr *)data;
    if (cksum16((uint16_t *)data, len, 0) != 0x0000) {
    ...
    case ICMP_TYPE_ECHO:
        ...
        // Exercise 11-3
        if (icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, hdr->values, (uint8_t *)hdr + ICMP_HDR_SIZE, len - ICMP_HDR_SIZE, iface->unicast, src) == -1) {
