...
arp_cache_delete(struct arp_cache *cache)
{
    ...
    cache->pa = 0;
    memset(cache->ha, 0x00, ETHER_ADDR_LEN);
...
arp_request(struct net_iface *iface, ip_addr_t tpa)
{
    ...
    memset(request.tha, 0x00, ETHER_ADDR_LEN);
...
arp_init(void)
{
    ...
    // Exercise 13-4
    if (net_protocol_register(NET_PROTOCOL_TYPE_ARP, arp_input)) {
