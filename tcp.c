...
struct tcp_queue_entry {
    ...
    uint32_t seq; // シーケンス番号と
    uint8_t flg; // 制御フラッグは記録しておく
    size_t len;
    uint8_t data[];
};
...
tcp_output(struct tcp_pcb *pcb, uint8_t flg, uint8_t *data, size_t len)
{
    ...
    if (TCP_FLG_ISSET(flg, TCP_FLG_SYN | TCP_FLG_FIN) || len) {
        tcp_retransmit_queue_add(pcb, seq, flg, data, len); // シーケンス番号を消費するものは再送対象。ack, reset等は対象外
    }
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    debugf("%s:%d => %s:%d, len=%zu (payload=%zu)",
        ...
        len, len - hlen); // payloadのサイズを len - sizeof(*hdr) としてしまうとtcpのオプションあった時おかしくなる
    tcp_dump(data, len);
    local.addr = dst;
    local.port = hdr->dst;
    foreign.addr = src;
    ...
    if (TCP_FLG_ISSET(hdr->flg, TCP_FLG_SYN)) {
        seg.len++; /* SYN flag consumes one sequence number(payloadが0でもackの計算ができるようにかな？ ack = seg.seq + seg.len) */
    }
...
RETRY:
    switch (pcb->state) {
    case TCP_PCB_STATE_ESTABLISHED:
        remain = sizeof(pcb->buf) - pcb->rcv.wnd; // pcb->rcv.wndの初期値はsizeof(pcb->buf)なので、remainはバッファにあるけど読めてない分という意味
        if (!remain) {
            if (sched_sleep(&pcb->ctx, &mutex, NULL) == -1) {
                debugf("interrupted");
                mutex_unlock(&mutex);
